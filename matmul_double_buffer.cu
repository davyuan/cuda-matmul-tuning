...
__device__ __forceinline__ void issue_load(
    ...
    int tile_idx, int row, int col, int inner_row_A, int inner_col_A, int inner_row_B, int inner_col_B, int K, int N){
    ...
    for(int i = 0; i < MINI_TILE_ROWS; i++){
        for(int j = 0; j < MINI_TILE_COLS; j+=4){
            if(inner_col_A + j + 3 < TILE_COLS_A){
                reinterpret_cast<float4*>(dstA_tile + (inner_row_A + i) * TILE_COLS_A + inner_col_A + j)[0] = 
                    reinterpret_cast<float4*>(A + global_offset_A + j)[0];         
            }

            if(inner_row_B + i < TILE_ROWS_B){
                reinterpret_cast<float4*>(dstB_tile + (inner_row_B + i) * TILE_COLS_B + inner_col_B + j)[0] = 
...
__device__ __forceinline__ void issue_load(
    float* A, float* B, float* dstA_tile, float* dstB_tile, 
    int tile_idx, int tid, int stride, int K, int N) {
    ...
    for (int i = tid; i < (TILE_ROWS_A * v_T_COLS_A); i += stride) {
        ...
        reinterpret_cast<float4*>(dstA_tile + i)[0] = *current_srcA;
    }
    ...
    for (int i = tid; i < (TILE_ROWS_B * v_T_COLS_B); i += stride) {
        ...
        reinterpret_cast<float4*>(dstB_tile + i)[0] = *current_srcB;
    }
}

__global__ void matmul_double_buffer(float* A, float* B, float* C, int M, int N, int K) {
    ...
    //issue_load(A, B, &(sharedA[buffer_idx][0][0]), &(sharedB[buffer_idx][0][0]), 0, tid, load_threads, K, N);   
    issue_load(A, B, &(sharedA[buffer_idx][0][0]), &(sharedB[buffer_idx][0][0]), 0, 
        row,  col, inner_row_A, inner_col_A, inner_row_B, inner_col_B, K, N);    
    __syncthreads(); // Wait for tile 0 to be fully loaded
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 0; i < 1; i++) {
        ...
        matmul_double_buffer<<<gridDim, blockDim>>>(d_A, d_B, d_C, M_pad, N_pad, K_pad);
