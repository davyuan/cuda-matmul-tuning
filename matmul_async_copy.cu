...
__global__ void matmul_async_copy(float* A, float* B, float* C, int M, int N, int K) {
    __shared__ alignas(16) float sharedA[2][TILE_ROWS_A][TILE_COLS_A];
    __shared__ alignas(16) float sharedB[2][TILE_ROWS_B][TILE_COLS_B];
    extern __shared__ uint8_t smem_barrier0[];
    extern __shared__ uint8_t smem_barrier1[];
    auto* async_barrier0 = reinterpret_cast<cuda::barrier<cuda::thread_scope::thread_scope_block>*>(smem_barrier0);
    auto* async_barrier1 = reinterpret_cast<cuda::barrier<cuda::thread_scope::thread_scope_block>*>(smem_barrier1);
    ...
    if (tid == 0) {
        init(async_barrier0, blockDim.x * blockDim.y);
        init(async_barrier1, blockDim.x * blockDim.y);
    }
    __syncthreads();

    int buffer_idx = 0; // Indicates which shared memory buffer to use
    issue_load(
        A, B, &(sharedA[buffer_idx][0][0]), &(sharedB[buffer_idx][0][0]), 0, 
        *async_barrier0, tid, load_threads, K, N);
...
#pragma unroll
        ...
        // Wait for all computation to finish on the current tile before the next load starts.
        auto temp = async_barrier1;
        async_barrier1 = async_barrier0;
        async_barrier0 = temp;
    }
    
    int final_read_idx = buffer_idx; 
    async_barrier0->arrive_and_wait();

    #pragma unroll
    for(int k=0; k < TILE_COLS_A; ++k) {
        for(int i=0; i < MINI_TILE_ROWS; ++i) {
            fragA[i] = sharedA[final_read_idx][inner_row_A + i][k];
        }
        for(int j=0; j < MINI_TILE_COLS; ++j) {
            fragB[j] = sharedB[final_read_idx][k][inner_col_B + j];
        }
        for(int i=0; i < MINI_TILE_ROWS; ++i) {
            for(int j=0; j < MINI_TILE_COLS; ++j) {
                threadResults[i][j] += fragA[i] * fragB[j];
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 0; i < 1; i++) {
        ...
        matmul_async_copy<<<gridDim, blockDim>>>(d_A, d_B, d_C, M_pad, N_pad, K_pad);
